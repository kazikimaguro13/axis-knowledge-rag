...
> **v0.7 以降の挙動変更 (spec_031 / Parent Document Retrieval)**: `parent_doc.enabled=true`
> (default) の index で検索した場合、`id` は H2 セクション粒度の `parent_id`
> (`{doc_id}#{slug}` 形式) になる。1 つの `.md` ファイルから複数の H2 セクションが
> hit すると、`results[]` に **同じ `path` (= file)** が複数行登場し得る。
> Pydantic スキーマ (`SearchResultPayload`) は変えていないので既存クライアントは
> そのまま動作する。
...
RAG 回答生成。Claude API を叩き、出典 `[N]` (1-indexed、sources の位置と一致) 付きで回答する。ANTHROPIC_API_KEY 未設定時は DUMMY モード（決定論的応答）。
