...
`mcp_server/server.py` は薄い wrapper に徹している。`search.py` / `rag.py` / `integrity.py` は既に「ライブラリとして使える」設計なので、コード重複ゼロで 5 tools を実装できた。
  ...
  "axes": [
    {
      ...
      "type": "enum",
      "required": true,
      "values": ["技術記事", "日記", "読書メモ", "ミーティングメモ", "アイデア"]
    },
    {"name": "topic", "type": "string"},
    {"name": "level", "type": "enum", "values": ["入門", "初級", "中級", "上級"]},
...
#    - "List tools" → 5 つの axis_* tools が表示される
...
├── server.py            # FastMCP サーバー本体 (5 tools)
├── schemas.py           # Pydantic 入力モデル
├── formatters.py        # Markdown / JSON 整形ヘルパー
└── tests/
    ├── __init__.py
    └── test_server.py   # pytest smoke tests (DUMMY mode, 18 tests)
```
