...
- [design-decisions.md](design-decisions.md) — 主要な設計判断 (ADR) 17 件
- [adr/ADR-017-parent-document-retrieval.md](adr/ADR-017-parent-document-retrieval.md) — Parent Document Retrieval (Small-to-Big) 採用の ADR (spec_031)
- [adr/ADR-022-session-persistence.md](adr/ADR-022-session-persistence.md) — Pluggable Session Persistence (Memory / SQLite / Redis) (spec_036)
- [api-reference.md](api-reference.md) — HTTP エンドポイント仕様 (FastAPI / v0.3 最終版)
- [search-fusion.md](search-fusion.md) — 3-way hybrid 検索 (軸 + ベクトル + BM25) アーキ図 + `bm25_weight` チューニングガイド
...
- [mcp-server.md](mcp-server.md) — MCP server (stdio) — 6 tools、Claude Desktop / Cowork 組み込み手順
- [ingester.md](ingester.md) — メモ → YAML frontmatter Markdown の AI 変換 (単発 CLI / バッチ CLI / MCP tool)
