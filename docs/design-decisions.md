...
### Update (2026-05-14, spec_043) — normalize は embedding only

当初 (ADR-007 制定時) は normalizer (NFKC + カナ統一 + lowercase) を **search クエリと
indexed body の両方** に適用する設計だった。しかし spec_031 の parent_doc 化以降、
`scripts/build_index.py` が chunker に `d.normalized_body or d.body` を渡したことで
ChromaDB に保存される `parent.text` / `child.text` も normalized (ひらがな化) になり、
検索結果の表示テキストが読みづらい文字列で返ってくる問題が発生 (v0.8.1 demo 撮影で発覚)。

#### 修正方針

- `scripts/build_index.py`: chunker に渡すのは ORIGINAL body (`d.body`)
- embedding 計算時のみ child 毎に `normalizer(c.text)` を適用
- `parent.text` / `child.text` の **storage は原文**、embedding ベクトルだけ正規化版

これにより:

- 検索のリコール (表記ゆれ吸収) は維持 ✅
- 表示テキストは読みやすい原文に ✅
- 既存 v0.8.1 まで normalized で保存された index は `VectorStore.load_parents()` の
  起動時 heuristic で warning ログを出し、`--rebuild` を推奨する (data loss 回避のため
  自動 rebuild はしない)
...
## ADR-017: Parent Document Retrieval (Small-to-Big) を採用する
...
v0.6 までは 1 ファイル = 1 embedding で、長文ファイルでは局所質問にもファイル丸ごとが
hit して精度が落ち、出典 `[doc_NNN]` の粒度がユーザーに対して粗すぎた。

### Decision

「小さく検索 (child)、大きく回答 (parent)」へ切り替える。`backend/src/chunker.py` を新設し:

- **parent** = H2 セクション (or H2 が無いなら doc 全体)
- **child** = parent 内部を H3+ / 段落 / 文末で切った ~256 token 小ブロック

ChromaDB には child だけを embedding し、parent は ChromaDB ディレクトリ直下の
`parents.json` sidecar に永続化。検索時は child の上位 `top_k_children=20` を取り、
`parent_id` で dedup → 上位 `top_n_parents=5` parent を返す。
LLM コンテキストは parent 全文を `build_context()` で連結 (`max_chars=8000` cap)。

### Consequences

- ✅ 精度向上 (関連 H2 セクションだけ抽出)、出典の粒度が H2 単位に
- ✅ LLM コンテキストが構造的に閉じる (H2 セクション本文単位)
- ✅ 既存 BM25 ハイブリッド (ADR-016) と併存可。BM25 は file-level スコア、parent への伝播は max(parent_score) で吸収
- ❌ `parents.json` という追加成果物。`build_index.py --rebuild --mode parent_doc` を 1 回必要
- ❌ BM25 (file 単位) と vector (parent 単位) のスコア粒度が違う (重み再調整は v0.7 リリース後の bench 課題)
- ❌ MCP の `sources` 配列で同じ `doc_id` (= file) が複数回現れ得る (異なる H2 から hit の場合)

### Alternatives Considered

- (a) 旧方式 (file 全文): 精度問題が顕在化済み → 却下
- (b) **RecursiveCharacterTextSplitter (LangChain)**: 依存追加 + Markdown 構造を捨てる → 却下
- (c) **Hierarchical Embeddings** (parent も child も embed): コスト 2 倍 → 却下
- (d) **採用**: child のみ embedding + parent は JSON sidecar

詳細: [docs/adr/ADR-017-parent-document-retrieval.md](adr/ADR-017-parent-document-retrieval.md)
